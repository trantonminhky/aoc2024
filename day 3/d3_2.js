...
for (let charIndex = 0; charIndex < corruptedMem.length - 3; charIndex++) {
    if ([corruptedMem[charIndex], corruptedMem[charIndex + 1], corruptedMem[charIndex + 2], corruptedMem[charIndex + 3]].join('') === 'do()') {
        enabled = true;
    } else if ([corruptedMem[charIndex], corruptedMem[charIndex + 1], corruptedMem[charIndex + 2], corruptedMem[charIndex + 3], corruptedMem[charIndex + 4], corruptedMem[charIndex + 5], corruptedMem[charIndex + 6]].join('') === 'don\'t()') {
        enabled = false;
    }
